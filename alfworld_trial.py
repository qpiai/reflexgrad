...
def textgrad_backward(policy: ActionPolicy, action: str, loss_text: str, model, context: dict) -> str:
    ...
    # Format tried actions history (last 10 to avoid huge context)
    if tried_actions:
        recent_tried = list(tried_actions)[-10:]
        tried_actions_str = "\n".join([f"  - {a}" for a in recent_tried])
    else:
...
RECENT ACTION HISTORY (already tried - don't recommend these again):
{tried_actions_str}
...
Suggested next step: {step_gradient.get('next_action_guidance', 'Unknown')}
(This is one perspective to consider along with task requirements and other context)
...
4. Use fuzzy scores as GUIDANCE to identify semantic matches
   ...
   - Low score but semantically correct: Still valid if it achieves the same goal
        ...
        for idx, (state, action) in enumerate(zip(active_states_all, selected_actions)):
            ...
            if policy is not None and loss_fn is not None and optimizer is not None:
                ...
                textgrad_context = {
                    ...
                    'tried_actions': state['tried_actions'],  # Will fail if missing
