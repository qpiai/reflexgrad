"""vLLM model wrapper — OpenAI-compatible API with vLLM base URL

Optimized for DeepSeek V3.2 Speciale via vLLM:
...
class vLLMModelWrapper:
    def __init__(self, model_name: str = None):
        ...
        print(f"Using vLLM model: {self.model_name}")
        if not client.api_key or client.api_key == "dummy_key":
            raise RuntimeError("VLLM_API_KEY not set.")
            ...
            try:
                ...
                if not result or len(result.strip()) == 0:
                    if attempt < max_retries - 1:
                        ...
                        print(f"[WARNING] Empty response from vLLM (attempt {attempt + 1}/{max_retries}), retrying in {wait_time:.1f}s...")
                        time.sleep(wait_time)
                        continue
                    else:
                        # GRACEFUL FALLBACK: Return a minimal valid response instead of crashing
                        print(f"[WARNING] vLLM returned empty response after {max_retries} attempts — returning fallback")
                ...
                if attempt < max_retries - 1:
                    ...
                    if is_rate_limit:
                        ...
                        print(f"[RATE LIMIT] vLLM 429 (attempt {attempt + 1}/{max_retries}), backing off {wait_time:.1f}s...")
                    else:
                        wait_time = (attempt + 1) * 3 + random.uniform(0, 2)
                        print(f"[WARNING] vLLM call failed (attempt {attempt + 1}/{max_retries}): {e}, retrying in {wait_time:.1f}s...")
                    time.sleep(wait_time)
                else:
                    # GRACEFUL FALLBACK: Don't crash the entire run
                    print(f"[ERROR] vLLM API failed after {max_retries} attempts: {e}")
...
class FastModelWrapper:
    ...
    def __init__(self):
        ...
        print(f"Using fast extraction model (vLLM): {self.model_name}")
    ...
    def generate(self, prompts, sampling_params=None):
        ...
        outs = [MockOutput(text or "[empty]") for text in results]
        elapsed = time.time() - start
        print(f"[FAST MODEL vLLM] {len(prompts)} prompts in {elapsed:.2f}s")
        return outs
...
model = vLLMModelWrapper(model_name=_default_model)
LLM = vLLMModelWrapper
print(f"vLLM {_default_model} model loaded successfully!")
